...
- 「确定没扣」与「结果未知」的边界：`4xx` 且带**约定的业务 `error.code`**（`INSUFFICIENT_BALANCE`、`USER_NOT_FOUND`、`UNAUTHORIZED` 三者之一）= 确定没扣；约定之外的自定义业务码会被 bot 当「结果未知」处理（宁可多试不可丢钱），请不要使用；**`408`/`409`/`425`/`429`（无业务码）与 `5xx`、无响应 = 结果未知**（请求可能已在网站生效，限流同样不保证未处理）——bot 对结果未知一律用同一 `ref` 重试收敛，网站端必须与 5xx 一样保证重试安全。
- 其余**无业务码的裸 `4xx`**：bot 按「确定没扣」处理（直接作废、不退款）——网站必须保证返回这类响应时扣款确实未发生；尤其不要返回无业务码的裸 `499`（语义上属「结果未知」），需要表达超时请返回 `408`，需要表达业务拒绝请带 `error.code`。
...
> `408`/`409`/`425`/`429`（无业务码）与 `5xx`、无响应一律按「结果未知」处理：bot 会用同一 `ref` 重试，网站端必须保证重试安全。
