...
- 「确定没扣」与「结果未知」的边界：`4xx` 且带业务 `error.code`（如 `INSUFFICIENT_BALANCE`、`USER_NOT_FOUND`）= 确定没扣；**`408`/`409`/`425`/`429`（无业务码）与 `5xx`、无响应 = 结果未知**（请求可能已在网站生效，限流同样不保证未处理）——bot 对结果未知一律用同一 `ref` 重试收敛，网站端必须与 5xx 一样保证重试安全。
